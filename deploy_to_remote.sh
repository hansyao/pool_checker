...
echo -e "开始远程添加环境变量"
REMOTE_CMD='cat ~/.bashrc | grep pool_checker; if [[ $? -eq 1 ]]; then echo -e "if [ -f ~/pool_checker/.profile ]; then . ~/pool_checker/.profile; fi" >> ~/.bashrc; fi; . ~/.bashrc'
remote_exec ${HOST_IP} ${SERVER_PORT} \
	${USER_NAME} ${AUTH} "${SSH_KEY}" "${REMOTE_CMD}"

echo -e "为远程服务器配置定时任务"
REMOTE_CMD='crontab -l | grep pool_checker; if [[ $? -eq 1 ]]; then crontab -l | { cat; echo -e "40 */1 * * * source `pwd`/.bashrc && cd `pwd`/pool_checker && ./start.sh clean && ./start.sh run >pool.log 2>&1 &"; } | crontab -; fi'
remote_exec ${HOST_IP} ${SERVER_PORT} \
	${USER_NAME} ${AUTH} "${SSH_KEY}" "${REMOTE_CMD}"